/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const __wbg_hullproxy_free: (a: number, b: number) => void;
export const __wbg_meshproxy_free: (a: number, b: number) => void;
export const color_diff: (a: number, b: number) => number;
export const find_direct_equivalences: (a: number) => [number, number, number];
export const get_hull: (a: number, b: number) => [number, number, number];
export const get_srgb_mesh: (a: number) => number;
export const hullproxy_add: (a: number, b: number) => void;
export const hullproxy_mesh: (a: number) => number;
export const hullproxy_new: (a: number, b: number) => [number, number, number];
export const init_panic_hook: () => void;
export const init_searcher: (a: number, b: number, c: number, d: number) => [number, number];
export const list_paints: () => [number, number, number];
export const meshproxy_colors: (a: number) => any;
export const meshproxy_indices: (a: number) => any;
export const meshproxy_positions: (a: number) => any;
export const search: (a: number, b: any) => [number, number, number];
export const __wbindgen_malloc: (a: number, b: number) => number;
export const __wbindgen_realloc: (a: number, b: number, c: number, d: number) => number;
export const __wbindgen_exn_store: (a: number) => void;
export const __externref_table_alloc: () => number;
export const __wbindgen_externrefs: WebAssembly.Table;
export const __wbindgen_free: (a: number, b: number, c: number) => void;
export const __externref_table_dealloc: (a: number) => void;
export const __wbindgen_start: () => void;
